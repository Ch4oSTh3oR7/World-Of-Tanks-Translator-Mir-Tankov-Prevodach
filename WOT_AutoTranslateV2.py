...
async def translate_msgstr_with_context(entry, translator):
    """
    Translate the msgstr asynchronously using the global source and target languages,
    incorporating context from the msgctxt field.
    """
    context = entry.msgctxt or "General context"
    msgid = entry.msgid
    msgstr = entry.msgstr

    # Prepare the text to provide context for translation
    full_text = f"Context: {context}\nOriginal: {msgid}\nTranslation Target: {msgstr}"

    try:
        translated = await translator.translate(full_text, src=SOURCE_LANG, dest=TARGET_LANG)
        translated_text = translated.text

        # Extract the translation for the target
        if "Translation Target:" in translated_text:
            return translated_text.split("Translation Target:")[1].strip()
        else:
            # Log a warning and return the entire translation if the structure is unexpected
            print(f"Warning: Unexpected translation format for entry with msgid '{msgid}'")
            return translated_text
    except Exception as e:
        print(f"Error translating entry with msgid '{msgid}': {e}")
        return msgstr  # Fallback to the original msgstr if translation fails


async def translate_po_file(input_po_path, output_po_path):
    """
    Translates the 'msgstr' parts of a .po file using Google Translate,
    while leveraging the msgctxt field for context.
    """
    ...
    for entry in po:
        if entry.msgstr:  # Only translate non-empty msgstr fields
            tasks.append(translate_msgstr_with_context(entry, translator))
